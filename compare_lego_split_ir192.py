#!/usr/bin/env python3
"""
Mapas 3D LEGO: Primarias ARRIBA, Secundarias ABAJO en subgráficas - Ir-192 200M
...
CASES = {
    "Pulmón MIRD\n(0.2958 g/cm³)": {
        "primary": None,  # No disponible aún
        "secondary": None,
    },
    "Hueso\n(1.85 g/cm³)": {
...
def mask_inner_region(data, exclude_radius_mm):
    """Crear máscara que excluye región interna circular"""
    ny, nx = data.shape
    yy, xx = np.ogrid[:ny, :nx]
    
    dist = np.sqrt((xx - SOURCE_CENTER_X)**2 + (yy - SOURCE_CENTER_Y)**2)
    exclude_radius_bins = exclude_radius_mm / BIN_SIZE_MM
    
    mask = dist >= exclude_radius_bins
    return mask

def subsample_data(data, factor=3):
    """Submuestrear datos por factor"""
    return data[::factor, ::factor]

def main():
    print("="*80)
    print("MAPAS 3D LEGO: PRIMARIAS (arriba) vs SECUNDARIAS (abajo) - Ir-192 200M")
    print("="*80)
    ...
    for case_label in CASES.keys():
        ...
        if CASES[case_label]['primary'] is None:
            print(f"  ⚠ Archivos no disponibles para este material")
            continue
        
        # Cargar primarias
        hist_name_prim = "h2_eDepPrimary"
        dose_primary = load_histogram(CASES[case_label]['primary'], hist_name_prim)
        
        # Cargar secundarias
        hist_name_sec = "h2_eDepSecondary"
        dose_secondary = load_histogram(CASES[case_label]['secondary'], hist_name_sec)
        
        if dose_primary is None or dose_secondary is None:
            print(f"  ❌ No se pudieron cargar los histogramas")
            continue
        
        # Convertir a dosis (Gy) con densidad específica del material
        density = MATERIAL_DENSITIES[case_label]
        dose_primary = np.vectorize(lambda x: edep_to_dose_gy(x, density))(dose_primary)
        dose_secondary = np.vectorize(lambda x: edep_to_dose_gy(x, density))(dose_secondary)
    ...
    print(f"\n{'='*70}")
    print(f"CONTRIBUCIÓN RELATIVA DE DOSIS PRIMARIA vs SECUNDARIA - Ir-192 200M")
    print(f"{'='*70}")
...
def create_split_lego_plots(data_maps, scales):
    ...
    for idx, case_label in enumerate(case_order):
        ...
        # Convertir a mGy - escala LINEAL para mejor visualizar diferencias
        dz_sec_mgy = secondary_sub.flatten() * 1000
        
        zpos_sec = np.zeros_like(xpos_flat)
        
        # Normalizar colores por máximo local en escala lineal
        max_sec_mgy = np.max(dz_sec_mgy)
        linear_max_sec = 4000.0  # 4000 mGy límite
        colors_norm_sec = dz_sec_mgy / linear_max_sec
        colors_norm_sec = np.clip(colors_norm_sec, 0, 1)
        
        cmap_sec = cm.get_cmap('Blues')
        colors_sec = cmap_sec(colors_norm_sec * 0.7 + 0.2)
        
        ax2.bar3d(xpos_flat, ypos_flat, zpos_sec, dx, dy, dz_sec_mgy, 
                 color=colors_sec, zsort='average', shade=True, alpha=0.8)
        
        # Agregar mapa 2D en la cara superior (en escala lineal mGy)
        secondary_sub_mgy = secondary_sub * 1000
        ax2.contourf(xposn, yposn, secondary_sub_mgy, levels=15, cmap='Blues', alpha=0.5, offset=linear_max_sec, zdir='z')
        
        ax2.set_xlabel('X (bins × 3)', fontsize=10)
        ax2.set_ylabel('Y (bins × 3)', fontsize=10)
        ax2.set_zlabel('Dosis [mGy]', fontsize=10)
        ax2.set_zlim(0, linear_max_sec)
        ...
        output_file = f'/home/fer/fer/newbrachy/lego_split_ir192_{idx+1}_{case_label.split(chr(10))[0].replace(" ", "_").lower()}.png'
...
def create_split_lego_comparison(data_maps, scales):
    """Crear figura comparativa con primarias ARRIBA, secundarias ABAJO para los casos"""
    
    print(f"\nGenerando comparación de materiales Ir-192 con primarias y secundarias...")
    
    log_min = 0.0
    log_max_primary = np.log10(scales['primary_max'])
    
    # Figura con 2 filas (primarias y secundarias) × N columnas (materiales)
    num_materials = len(data_maps)
    fig = plt.figure(figsize=(8*num_materials, 12))
    
    # Usar GridSpec para control de espaciado vertical entre filas
    gs = GridSpec(2, num_materials, figure=fig, hspace=0.35, wspace=0.3)
    ...
    for col_idx, case_label in enumerate(case_order):
        ...
        # Normalizar colores en escala lineal
        linear_max_sec = 4000.0  # 4000 mGy límite
        colors_norm_sec = dz_sec_mgy / linear_max_sec
        colors_norm_sec = np.clip(colors_norm_sec, 0, 1)
        ...
        ax.bar3d(xpos_flat, ypos_flat, zpos_sec, dx, dy, dz_sec_mgy, 
                color=colors_sec, zsort='average', shade=True, alpha=0.8)
        
        # Agregar mapa 2D en la cara superior (en escala lineal mGy)
        secondary_sub_mgy = secondary_sub * 1000
        ax.contourf(xposn, yposn, secondary_sub_mgy, levels=15, cmap='Blues', alpha=0.5, offset=linear_max_sec, zdir='z')
        
        ax.set_xlabel('X (×3)', fontsize=9)
        ax.set_ylabel('Y (×3)', fontsize=9)
        ax.set_zlabel('Dosis [mGy]', fontsize=9)
        material_name_es = MATERIAL_NAMES_ES.get(case_label, case_label)
        ax.set_title(material_name_es, fontsize=11, fontweight='bold', color='darkblue')
        ax.set_zlim(0, linear_max_sec)
        ax.view_init(elev=25, azim=45)
    ...
    output_file = '/home/fer/fer/newbrachy/lego_split_ir192_comparison.png'
    plt.savefig(output_file, dpi=120, bbox_inches='tight')
    print(f"  ✓ Guardado: {output_file}")
    print(f"    Tamaño: {np.round(os.path.getsize(output_file) / 1e6, 2)} MB")
    plt.close()
