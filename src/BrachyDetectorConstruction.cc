  ...
  fDetectorChoice(0)
{
 ...
 fWorldSizeZ = 4.0*m;
  ...
  fHeterogeneityEnabled = false;
  fHeterogeneityMaterialName = "G4_WATER";
  fHeterogeneityHalfLengths = G4ThreeVector(5.*mm, 5.*mm, 5.*mm);
  fHeterogeneityPosition = G4ThreeVector();
...
void BrachyDetectorConstruction::EnableHeterogeneity(G4bool enable)
{
...
void BrachyDetectorConstruction::SetHeterogeneityMaterial(const G4String& materialName)
{
  ...
  if (!nist->FindOrBuildMaterial(materialName)) {
    G4cout << "WARNING: heterogeneity material '" << materialName << "' not available!" << G4endl;
    return;
  }
...
void BrachyDetectorConstruction::SetHeterogeneitySize(const G4ThreeVector& size)
{
  if (size.x() <= 0. || size.y() <= 0. || size.z() <= 0.) {
    G4cout << "WARNING: heterogeneity size must be positive in all directions." << G4endl;
    return;
  }
...
void BrachyDetectorConstruction::SetHeterogeneityPosition(const G4ThreeVector& position)
{
...
void BrachyDetectorConstruction::BuildHeterogeneity()
{
  ...
  if (!material) {
    G4cout << "WARNING: heterogeneity material '" << fHeterogeneityMaterialName << "' not available. Disabling inclusion." << G4endl;
    fHeterogeneityEnabled = false;
    return;
  }

  const G4ThreeVector extents = G4ThreeVector(2. * fHeterogeneityHalfLengths.x(),
                                             2. * fHeterogeneityHalfLengths.y(),
                                             2. * fHeterogeneityHalfLengths.z());

  const auto exceedsX = std::abs(fHeterogeneityPosition.x()) + fHeterogeneityHalfLengths.x() > fPhantomSizeX;
  const auto exceedsY = std::abs(fHeterogeneityPosition.y()) + fHeterogeneityHalfLengths.y() > fPhantomSizeY;
  const auto exceedsZ = std::abs(fHeterogeneityPosition.z()) + fHeterogeneityHalfLengths.z() > fPhantomSizeZ;
  if (exceedsX || exceedsY || exceedsZ) {
    G4cout << "WARNING: heterogeneity volume (" << extents << ") placed at "
           << fHeterogeneityPosition << " does not fit inside the phantom. Disabling inclusion." << G4endl;
    fHeterogeneityEnabled = false;
    return;
  }

  fHeterogeneitySolid = new G4Box("HeterogeneitySolid",
                                  fHeterogeneityHalfLengths.x(),
                                  fHeterogeneityHalfLengths.y(),
                                  fHeterogeneityHalfLengths.z());

  fHeterogeneityLog = new G4LogicalVolume(fHeterogeneitySolid, material, "HeterogeneityLog", nullptr, nullptr, nullptr);

  fHeterogeneityPhys = new G4PVPlacement(nullptr,
                                         fHeterogeneityPosition,
                                         fHeterogeneityLog,
                                         "HeterogeneityPhys",
                                         fPhantomLog,
                                         false,
  ...
  auto visColour = G4Colour(0.90, 0.40, 0.10);
  auto* heteroVis = new G4VisAttributes(visColour);
